...
struct XP3FilterDecoder {
	...
	XP3FilterDecoder() : ManagedDecoder(nullptr), ManagedFilter(nullptr), ScriptEngine(new tTJS()) {
		ScriptEngine->SetConsoleOutput(TVPGetTJS2ConsoleOutputGateway());
	}
	~XP3FilterDecoder() {
...
NCB_INTERNAL_POST_REGIST_CALLBACK(TJS_W("xp3filter.dll"), PostRegistCallback);
